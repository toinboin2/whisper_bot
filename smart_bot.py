...
# Проверяем ключ Google
if not GOOGLE_API_KEY:
    print("❌ ОШИБКА: Не найден GOOGLE_API_KEY!")
else:
    genai.configure(api_key=GOOGLE_API_KEY)

# КАСКАДНАЯ СИСТЕМА МОДЕЛЕЙ
# Бот будет пробовать их по очереди, пока одна не сработает
MODEL_CASCADE = [
    "gemini-2.0-pro-exp-02-05",  # Самая умная (экспериментальная)
    "gemini-2.5-flash",          # Быстрая и новая
    "gemini-2.0-flash",          # Стандартная быстрая
    "gemini-1.5-pro",            # Стабильная мощная
    "gemini-1.5-flash"           # Запасная легкая
]

# Промпт (Инструкция для ИИ)
# Вы можете менять этот текст, чтобы ИИ вел себя по-другому
SYSTEM_PROMPT = (
    "Сделай полную и точную транскрибацию этого аудио. "
    "Раздели текст на абзацы. Обозначай спикеров (Спикер 1, Спикер 2). "
    "Исправь мелкие оговорки, но сохрани смысл. "
